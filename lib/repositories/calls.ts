...
import { sanitizePostgrestSearch } from "@/lib/domain/search";
...
export type CallsListFilters = {
  ...
  dateFrom?: string; dateTo?: string; outcome?: "won" | "lost" | "unknown";
  bottleneck?: string; objection?: string; executionMin?: number; executionMax?: number;
  leadQualityMin?: number; leadQualityMax?: number; needsReview?: boolean;
  sort?: "newest" | "oldest" | "execution_desc" | "execution_asc" | "lead_quality_desc" | "lead_quality_asc" | "duration_desc" | "talk_ratio_desc";
};

const sortColumns = {
  newest: ["date", false], oldest: ["date", true], execution_desc: ["seller_execution_score", false],
  execution_asc: ["seller_execution_score", true], lead_quality_desc: ["lead_quality_score", false],
  lead_quality_asc: ["lead_quality_score", true], duration_desc: ["duration", false], talk_ratio_desc: ["seller_talk_ratio", false],
} as const;
...
export async function listCalls(filters: CallsListFilters = {}) {
  ...
  if (search) query = query.or(`title.ilike.%${search}%,lead.ilike.%${search}%`);
  if (filters.closer) query = query.eq("closer", filters.closer);
  if (filters.dateFrom) query = query.gte("date", filters.dateFrom);
  if (filters.dateTo) query = query.lte("date", `${filters.dateTo}T23:59:59.999Z`);
  if (filters.outcome) query = query.eq("outcome", filters.outcome);
  if (filters.bottleneck) query = query.eq("bottleneck", filters.bottleneck);
  if (filters.objection) query = query.eq("dominant_objection", filters.objection);
  if (filters.executionMin !== undefined) query = query.gte("seller_execution_score", filters.executionMin);
  if (filters.executionMax !== undefined) query = query.lte("seller_execution_score", filters.executionMax);
  if (filters.leadQualityMin !== undefined) query = query.gte("lead_quality_score", filters.leadQualityMin);
  if (filters.leadQualityMax !== undefined) query = query.lte("lead_quality_score", filters.leadQualityMax);
  if (filters.needsReview !== undefined) query = query.eq("needs_review", filters.needsReview);
  const [sortColumn, ascending] = sortColumns[filters.sort ?? "newest"];
