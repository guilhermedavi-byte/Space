...
`;
  ...
  const phoneOrName = value => window.SpaceInternationalPhone?.formatPhoneForDisplay(value, {flag:false}) || value;
  const titleFor = row => phoneOrName(row?.contact?.name || row?.contact?.phone || 'Contato sem nome');
  ...
  const photoUrl = value => { if (typeof value !== 'string' || !value.trim()) return ''; try { const u = value.startsWith('/api/attendance-inbox/avatar?') ? new URL(value, window.location.origin) : new URL(value); return u.protocol === 'https:' && !u.username && !u.password ? u.href : ''; } catch { return ''; } };
  ...
  function renderAudio(msg, context = {}) {
    ...
    return `<div class="ai-audio" data-ai-audio="${esc(msg.message_id)}">
      ${avatar(msg.direction === 'outbound' ? context.author || {name:'Space'} : context.contact)}
  ...
  function renderMedia(msg, context) {
    ...
    if (msg.kind === 'audio') return renderAudio(msg, context);
  ...
  root.addEventListener('click',async event=>{
    ...
    try{
      if(tool){
        if(tool==='attach'){const input=root.querySelector('[data-composer-file]');if(input){input.value='';input.click();}return;}
