...
const handleLessonLogsApi = async (req, res, { idToken, role, requesterId, url } = {}) => {
  ...
  if (req.method === "GET" || req.method === "HEAD") {
    ...
    // List logs for a teacher (or a specific teacher for admin).
    try {
      const query = {
  ...
  // Build payload, omitting undefined values (Firestore doesn't accept them).
  const data = {
    eventId,
    ...
    engajamento: clampInt(Number(body?.engajamento) || 0, 0, 5) || null,
    notaAula: (() => {
  ...
  // Hide content/assessment when the status doesn't apply.
