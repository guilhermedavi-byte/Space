...
const {invalidateOverviewSnapshots}=require('./_lib/finance-dashboard-snapshot');
const {rebuildOverviewMonths,saoPauloMonth}=require('./_lib/finance-overview-rebuild');
...
async function readRows(connectionId=process.env.FINANCE_CONNECTION_ID){
 ...
 try{const directory=createAsaasClient({readOnly:true});for await(const page of directory.pages('customers',{limit:100,maxPages:100})){for(const c of page.data||[])if(c?.id&&c?.name)names.set(c.id,c.name);}}catch{}
