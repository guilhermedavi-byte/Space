...
async function readRows(connectionId=process.env.FINANCE_CONNECTION_ID){
 const c=uuid(connectionId),all=async(table,select)=>{const out=[];for(let offset=0;offset<20000;offset+=500){const r=await supabaseFetch(`/${table}?connection_id=eq.${c}&select=${select}&order=id&offset=${offset}&limit=500`);if(!Array.isArray(r.data))throw Error('finance_read_failed');out.push(...r.data);if(r.data.length<500)return out;}throw Error('finance_read_limit');};
 const [receivables,links]=await Promise.all([all('finance_receivables','id,asaas_payment_id,asaas_customer_id,status,value,due_date,billing_type,deleted,snapshot'),all('finance_customer_student_links','asaas_customer_id,firestore_doc_id')]);
 ...
 return receivables.map(r=>({...r,student_ids:linked.get(r.asaas_customer_id)||[],linked:Boolean(linked.get(r.asaas_customer_id)?.length)}));
}
module.exports=async(req,res)=>{
 ...
 try{const b=await body(req),rows=await readRows(),svc=createFinanceReconciliation({client:createAsaasClient({readOnly:false})});const actor=String(user.sub||user.uid||user.email||'operator');const result=b.action==='undo_reconciliation'?await svc.undo({body:b,actor}):await svc.confirm({body:b,actor,rows});return send(200,{ok:true,result});}
 catch(e){const safe=new Set(['finance_reconciliation_invalid','finance_reconciliation_classification_invalid','finance_reconciliation_not_found','finance_external_id_invalid','asaas_read_only','asaas_remote_error','asaas_forbidden','asaas_unauthorized']);return send(e.status|| (safe.has(e.code||e.message)?400:503),{error:safe.has(e.code||e.message)?(e.code||e.message):'finance_reconciliation_unavailable'});}
};
