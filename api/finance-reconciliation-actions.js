...
async function readRows(connectionId=process.env.FINANCE_CONNECTION_ID){
 ...
 const [receivables,payments,links]=await Promise.all([all('finance_receivables','id,asaas_payment_id,asaas_customer_id,status,value,due_date,billing_type,deleted,snapshot'),all('finance_payments','id,asaas_payment_id,status,value,payment_date,confirmed_date,refund_value'),all('finance_customer_student_links','asaas_customer_id,firestore_doc_id')]);
 ...
 return {rows:receivables.map(r=>({...r,student_ids:linked.get(r.asaas_customer_id)||[],linked:Boolean(linked.get(r.asaas_customer_id)?.length)})),payments};
}
