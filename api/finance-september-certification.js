...
const {cents,paymentCompetenceDate,isRevenueRow,originRuleMap,movementOrigin,PAID_STATUSES,CLOSED_STATUSES,REVENUE_STATUSES,AMBIGUOUS_BILLING_TYPES}=require('./_lib/finance-revenue-policy');
...
const brl=c=>Number.isSafeInteger(c)?c:null;
...
const txValue=t=>cents(String(Math.abs(Number(t.value??t.amount??0))).includes('.')?String(Math.abs(Number(t.value??t.amount??0)).toFixed(2)):String(Math.abs(Number(t.value??t.amount??0))))||0;
