...
function candidates({users,students,contracts},customers,subscriptions,existing=[]){
 ...
 const aliases=new Map();for(const student of valid.values())for(const key of ['firestoreDocId','uid','authUserId']){const value=text(student[key]);if(value){const ids=aliases.get(value)||new Set();ids.add(student.firestoreDocId);aliases.set(value,ids);}}
 const resolve=value=>aliases.get(value)?.size===1?[...aliases.get(value)][0]:null;
 for(const c of customers)if(!c.deleted)add(c.id,resolve(c.externalReference),'asaas_external_reference');
 for(const sub of subscriptions)if(!sub.deleted)add(sub.customer,resolve(sub.externalReference),'asaas_subscription_external_reference');
...
function profiles({users,students,contracts}){
 ...
 for(const s of users.filter(isStudent)){
  ...
  result.set(s.firestoreDocId,{student_id:s.firestoreDocId,name:text(s.nome||s.nomeCompleto||s.displayName||s.name),lifecycle_status:status,lifecycle_label:({active:'Ativo',cancellation_requested:'Cancelamento solicitado',cancellation_scheduled:'Em aviso prévio',churned:'Encerrado'})[status]||status,
   student_status:({active:'Ativo',inactive:'Inativo',ativo:'Ativo',inativo:'Inativo'})[text(s.status)]||text(s.status)||(typeof s.ativo==='boolean'?(s.ativo?'Ativo':'Inativo'):null),plans:[...new Set([...rows.map(r=>text(r.plan_name)),text(s.plano||s.plan)].filter(Boolean))],notice:rows.filter(r=>r.notice_started_at).map(r=>({started_at:r.notice_started_at,last_active_date:r.last_active_date||null,churn_at:r.churn_at||null})),cs_owner:text(s.responsavelCS||s.csResponsavel||s.cs_owner_name),source:own.length?'Space · contratos canônicos':'Space · Firestore'});
 }
 return result;
}
