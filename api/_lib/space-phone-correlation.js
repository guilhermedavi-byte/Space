...
const isCandidate = (call, score) => {
  if (conflicts(call, score)) return false;
...
const getCorrelatedScore = async (call, request) => {
  for (const [field, id] of [['call_leg_id', call.telnyx_call_leg_id], ['call_session_id', call.telnyx_call_session_id]]) {
    ...
    if (matches.length === 1) {
      ...
      if (conflicts(call, score)) return null;
      if (digits(call.from_number) && digits(score.from_number) && digits(call.from_number) !== digits(score.from_number)) return null;
      if (digits(call.to_number) && digits(score.to_number) && digits(call.to_number) !== digits(score.to_number)) return null;
      return { ...score, correlationMethod: field };
    }
  }
  const start = Date.parse(call.started_at);
  ...
  return { ...score, correlationMethod: 'unique_temporal' };
};
