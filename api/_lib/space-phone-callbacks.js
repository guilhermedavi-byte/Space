...
const scope = ({ user, isAdmin, sdr, eligibleSdrUids }) => {
  if (!user?.sub) throw fail('forbidden', 403);
  const uid = isAdmin ? (sdr && sdr !== 'all' ? sdr : '') : user.sub;
  if (uid) return `&space_user_uid=eq.${encodeURIComponent(uid)}`;
  if (isAdmin && Array.isArray(eligibleSdrUids)) {
    const uids = eligibleSdrUids.map(value => String(value || '').trim()).filter(Boolean);
    return uids.length ? `&space_user_uid=in.(${uids.map(encodeURIComponent).join(',')})` : '&space_user_uid=eq.__no_operational_sdr__';
  }
  return '';
};
...
async function list({ request, user, isAdmin, sdr, eligibleSdrUids }) {
  const result = rows(await request(`/voice_calls?select=${fields}&callback_at=not.is.null&callback_status=in.(scheduled,snoozed)${scope({user,isAdmin,sdr,eligibleSdrUids})}&order=callback_at.asc,id.asc&limit=1000`));
