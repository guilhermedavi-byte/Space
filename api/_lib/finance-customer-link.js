...
async function directory(){if(cached&&Date.now()<until)return cached;if(!pending)pending=(async()=>{const sources=await space.loadSources();const client=createAsaasClient({readOnly:true});await createFinanceFoundation({client,logger:()=>{}}).verifyConnection({recordHealth:false});const customers=[];for await(const p of client.pages('customers',{limit:100,maxPages:100}))customers.push(...p.data);const ids=[];for(let offset=0;offset<20000;offset+=500){const {data}=await supabaseFetch(`/finance_receivables?connection_id=eq.${process.env.FINANCE_CONNECTION_ID}&select=asaas_customer_id&order=id&offset=${offset}&limit=500`);ids.push(...data.map(r=>r.asaas_customer_id));if(data.length<500)break;if(offset===19500)throw Error('finance_read_limit');}await require('./finance-customer-directory').completeCustomers(customers,ids,client);const value={sources,customers};cached=value;until=Date.now()+120000;return value;})().finally(()=>pending=null);return pending;}
