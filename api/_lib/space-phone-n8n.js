...
const saveAiQualification = async ({ callId, qualification = {}, request = supabaseFetch }) => {
  ...
  const existing = await getQualification(call.id, request).catch(() => null);
  const merged = { ...body };
  if (!existing) merged.created_at = now;
  const rows = asRows(await request('/voice_call_qualifications?on_conflict=voice_call_id', { method: 'POST', headers: { Prefer: 'resolution=merge-duplicates,return=representation' }, body: merged, timeoutMs: 12000 }));
  return { ok: true, qualification: rows[0] || merged };
};
...
const dispatchQualificationEvent = async ({ event, callId, fetchImpl = fetch, now = new Date() } = {}) => {
  ...
  try {
    const response = await fetchImpl(url, { method: 'POST', redirect: 'error', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(payload) });
...
const datacrazyNote = async ({ callId, datacrazyId, note = '', request = supabaseFetch }) => {
  ...
  const base = clean(process.env.CRM_API_BASE_URL).replace(/\/+$/, '');
  const key = clean(process.env.CRM_API_KEY);
  if (!base || !key) return { ok: false, status: 503, error: 'datacrazy_write_env_missing' };
  // The existing codebase certifies Datacrazy read/mirror endpoints only. Do not invent a history/comment endpoint.
  return { ok: false, status: 501, error: 'DATACRAZY_NOTE_WRITE_BLOCKED_API_ENDPOINT', preparedNote: formatQualificationNote({ call, qualification }) };
};
