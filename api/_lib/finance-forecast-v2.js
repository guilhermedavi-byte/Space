...
function modelFromObservations(observations) {
  const normalized = (observations || []).map(o => ({ ...o, value: Number(o.value) || 0, eventDay: o.paid && Number.isFinite(o.eventDay) ? o.eventDay : null, censorDay: !o.paid && Number.isFinite(o.censorDay) ? o.censorDay : null })).filter(o => o.value > 0 && (o.eventDay != null || o.censorDay != null));
  const paid = normalized.filter(o => o.eventDay != null);
  const censored = normalized.filter(o => o.eventDay == null && o.censorDay != null);
  const sampleValue = sum(normalized);
  const paidValue = sum(paid);
  const censoredValue = sum(censored);
  const eventDays = [...new Set(paid.map(o => o.eventDay))].sort((a, b) => a - b);
  let survival = 1;
  const curve = [];
  for (const day of eventDays) {
    const atRisk = normalized.filter(o => (o.eventDay != null && o.eventDay >= day) || (o.censorDay != null && o.censorDay >= day));
    const atRiskValue = sum(atRisk);
    const events = paid.filter(o => o.eventDay === day);
    const eventValue = sum(events);
    if (atRiskValue > 0 && eventValue > 0) survival *= Math.max(0, 1 - eventValue / atRiskValue);
    curve.push({ day, survival, at_risk_count: atRisk.length, at_risk_value: atRiskValue, event_count: events.length, event_value: eventValue });
  }
  const survivalAt = day => { let value = 1; for (const point of curve) { if (point.day > day) break; value = point.survival; } return value; };
  const riskAt = day => normalized.filter(o => (o.eventDay != null && o.eventDay >= day) || (o.censorDay != null && o.censorDay >= day));
  const conditional = (currentDay, horizonDay) => {
    ...
    const risk = riskAt(currentDay);
    const riskValue = sum(risk);
    const events = paid.filter(o => o.eventDay > currentDay && o.eventDay <= horizonDay);
    const eventValue = sum(events);
    const sCurrent = survivalAt(currentDay), sHorizon = survivalAt(horizonDay);
    if (risk.length < MIN_SAMPLE || riskValue <= 0 || sCurrent <= 0) return { probability: null, risk_value: riskValue, event_value: eventValue, sample_count: risk.length, survival_current: sCurrent, survival_horizon: sHorizon };
    return { probability: clamp01(1 - (sHorizon / sCurrent)), risk_value: riskValue, event_value: eventValue, sample_count: risk.length, survival_current: sCurrent, survival_horizon: sHorizon };
  };
  const cumulative = horizonDay => {
    if (!normalized.length || sampleValue <= 0) return null;
    return clamp01(1 - survivalAt(horizonDay));
  };
  return { conditional, cumulative, survivalAt, riskAt, curve, sample_count: normalized.length, paid_sample_count: paid.length, censored_sample_count: censored.length, sample_value: sampleValue, paid_sample_value: paidValue, censored_sample_value: censoredValue };
}
...
module.exports = { MODEL_VERSION, signedDayDiff, buildFinanceForecastV2, writeForecastSnapshot, modelFromObservations };
