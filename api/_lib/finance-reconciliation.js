...
const NON_REVENUE=new Set(['pf_receivables_transfer','space_refund','capital_contribution','partner_loan','non_operational_movement','other']);
...
function createFinanceReconciliation({connectionId=process.env.FINANCE_CONNECTION_ID,client=createAsaasClient({readOnly:false}),foundation=createFinanceFoundation({client,logger:()=>{}})}={}){
 ...
 return {listCases,confirm,undo,buildMovements,candidateReceivables,buildFinancials};
}
