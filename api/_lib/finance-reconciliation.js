...
function createFinanceReconciliation({connectionId=process.env.FINANCE_CONNECTION_ID,client=createAsaasClient({readOnly:false}),foundation=createFinanceFoundation({client,logger:()=>{}})}={}){
 ...
 const classifyOrigin=async({body,actor,rows=[],payments=[]}={})=>{
  ...
  const cases=await listCases();const movements=buildMovements(rows,payments,cases).filter(m=>m.origin===origin&&m.status==='pending'&&!m.value_allocated);
  ...
  const docs=[];for(const m of selected){const before=await readCase(m.id),prior=before&&typeof before==='object'?before:{};const status='classified';const next={...prior,connection_id:uuid(connectionId),movement_id:m.id,status,classification,origin,allocations:[],updated_at:now,created_at:prior.created_at||now,events:[...(Array.isArray(prior.events)?prior.events:[]),event('classify_origin_batch',actor,{status,classification,origin},body?.note)].slice(-100)};docs.push({movementId:m.id,data:next});}
