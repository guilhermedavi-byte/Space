...
function createHandler({session=getSessionFromRequest,reader,env=process.env}={}){
 ...
 return async(req,res)=>{
  ...
  try{const bearer=String(req.headers?.authorization||'').match(/^Bearer\s+(.+)$/i)?.[1]||'';const commercialGoalLoader=view==='overview'?async(month)=>{const host=String(req.headers?.host||'');if(!host)return null;const proto=String(req.headers?.['x-forwarded-proto']||'https').split(',')[0].trim()||'https';const url=`${proto}://${host}/api/growth-dashboard?api=growth-goals&mode=management&competencia=${encodeURIComponent(month)}`;const headers={Accept:'application/json',Cookie:String(req.headers?.cookie||'')};if(bearer)headers.Authorization=`Bearer ${bearer}`;const r=await fetch(url,{headers});if(!r.ok)return null;return await r.json();}:null;const activeReader=reader||(view==='overview'?createReader({commercialGoalIdToken:bearer,commercialGoalLoader}):(service ||= createReader()));const result=await perf.measure('financeRead',()=>activeReader.get(view,q));return send(result.not_found?404:200,result);}
