...
function createHandler({session=getSessionFromRequest,reader,env=process.env}={}){
 ...
 return async(req,res)=>{
  ...
  const user=session(req);if(!user)return send(401,{error:'unauthorized'});if(!canAccessFinance(user))return send(403,{error:'forbidden'});
  ...
  if(!['overview','receivables','subscriptions','customers','receivable'].includes(view)||Object.values(q).some(v=>v.length>160)||q.month&&!/^\d{4}-(0[1-9]|1[0-2])$/.test(q.month)||q.page&&(!/^\d{1,4}$/.test(q.page)||Number(q.page)<1)||['from','to'].some(k=>q[k]&&(!/^\d{4}-\d{2}-\d{2}$/.test(q[k])||Number.isNaN(Date.parse(q[k]))||new Date(q[k]).toISOString().slice(0,10)!==q[k]))||q.from&&q.to&&q.from>q.to)return send(400,{error:'finance_filter_invalid'});
