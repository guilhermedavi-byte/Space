...
const deleteAuthUserBestEffort = async (uid) => {
  ...
  try {
    ...
    console.warn("[admin-create-user] auth rollback failed", { uid: localId, message: error?.message || String(error || "") });
  }
};
...
const createAdminFirestoreDoc = async ({ uid, nome, email, createdBy }) => {
  ...
  const doc = {
    ...
    atualizadoEm: nowIso,
    updatedAt: nowIso,
    source: "admin_access_management",
  };
  ...
  if (!response.ok) {
    const error = new Error("firestore_create_admin_failed");
    error.code = "firestore_create_admin_failed";
    error.status = response.status;
...
const mapFirebaseAuthError = (error) => {
  ...
  return { status: 500, error: "auth_create_failed", message: "Não foi possível criar o usuário no Firebase Auth." };
};
...
module.exports = async (req, res) => {
  ...
  const auth = await resolveAdminRequestAuth(req, { logPrefix: "[admin-create-user]" });
  if (!auth.ok) {
    sendJson(res, auth.status, auth.body);
    return;
  }
  if (String(auth.session?.role || "") !== "admin") {
    sendJson(res, 403, { error: "forbidden", message: "Apenas administradores podem acessar esta ação." });
    return;
  }

  let requester = null;
  try {
    requester = await getDocumentAsAdmin(`users/${encodeURIComponent(auth.session.sub)}`);
  } catch (error) {
    console.error("[admin-create-user] requester profile load failed", error);
    sendJson(res, 403, { error: "forbidden", message: "Não foi possível validar o super admin." });
    return;
  }

  if (requester?.isSuperAdmin !== true) {
    sendJson(res, 403, { error: "super_admin_only", message: "Apenas o super admin pode criar novos administradores." });
    return;
  }
