...
function query(cursor){
  return {structuredQuery:{select:{fields:SELECT.map(fieldPath=>({fieldPath}))},from:[{collectionId:'sdrActivityEvents'}],
    where:{compositeFilter:{op:'AND',filters:[['GREATER_THAN_OR_EQUAL','2026-06-01'],['LESS_THAN_OR_EQUAL','2026-10-31']].map(([op,stringValue])=>({fieldFilter:{field:{fieldPath:'dateKey'},op,value:{stringValue}}}))}},
...
async function readSdrEvents({fetchImpl=global.fetch,getToken,decode}={}){
  const token=await getToken();const out=[],seen=new Set();let cursor;
  for(let page=0;page<200;page++){
    const response=await fetchImpl('https://firestore.googleapis.com/v1/'+PREFIX+':runQuery',{method:'POST',headers:{Authorization:'Bearer '+token,'Content-Type':'application/json'},body:JSON.stringify(query(cursor)),signal:AbortSignal.timeout(15000)});
