...
function project(audit,input,counts){
  ...
  for(const [key,m] of Object.entries(audit.months)){
    ...
    months[key]={invariants:Object.fromEntries(['estimated_value_deals','invalid_financial_deals','unverified_revenue_dates','improper_gaps','crm_live_metas_delta'].map(k=>[k,metric[k]])),sdrActivity:{available:m.sdr.available,unallocatedEvents:m.sdr.unallocated,missingEventIds:m.sdr.missingIds,meetingShows:m.sdr.partitions.reduce((n,p)=>n+p.metrics.total_meeting_shows,0),attributedEvents:m.sdr.partitions.reduce((n,p)=>n+p.metrics.attributed,0)},sourceDealCount:sourceSales.count,sourceRevenue:sourceSales.actualValue,monthlyDealCount:metric.deals,monthlyRevenue:metric.monthly_realized,weeklyDealCount:l.weekly.reduce((s,w)=>s+w.rows.length,0),weeklyRevenue:metric.weekly_realized,monthlyWeeklyFinancialDelta:metric.monthly_weekly_delta,monthlyWeeklyDealIdDelta:comparison(m.monthlyWeekly).dealIdDelta,duplicateDealCount:metric.duplicate_attributions,duplicateRevenue:metric.duplicate_attribution_revenue,unallocatedDealCount:metric.orphan_deals,unallocatedRevenue:metric.unallocated_revenue,
