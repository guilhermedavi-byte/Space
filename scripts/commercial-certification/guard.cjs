...
function createGuard({env,fetchImpl=global.fetch}){
  ...
  const collections=new Set(['growthGoals','growthPeople','growthConfig','users','sdrActivityEvents','crmLiveCache','crmLiveSnapshots']);
  ...
  const fetchReadOnly=async(input,options={})=>{
    ...
    const fsRead=firestore&&method==='GET'&&url.pathname.startsWith(firestorePrefix)&&collections.has(decodeURIComponent(url.pathname.slice(firestorePrefix.length)).split('/')[0])&&[...url.searchParams.keys()].every(k=>['pageSize','pageToken','mask.fieldPaths'].includes(k));
