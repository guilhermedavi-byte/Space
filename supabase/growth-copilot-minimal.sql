...
alter table public.growth_sales_scripts add column if not exists examples text;
alter table public.growth_sales_scripts add column if not exists when_to_use text;
...
alter table public.growth_winning_phrases add column if not exists context text;
alter table public.growth_winning_phrases add column if not exists stage text;
alter table public.growth_winning_phrases add column if not exists source text;
alter table public.growth_winning_phrases add column if not exists usage_count integer default 0;
...
delete from public.growth_sales_scripts where title in ('Primeiro contato / abertura da call', 'Qualificação do objetivo', 'Apresentação personalizada do plano', 'Posicionamento contra método tradicional', 'Tecnologia e acompanhamento', 'Fechamento consultivo', 'Urgência e condição especial', 'Onboarding após fechamento') or name in ('Primeiro contato / abertura da call', 'Qualificação do objetivo', 'Apresentação personalizada do plano', 'Posicionamento contra método tradicional', 'Tecnologia e acompanhamento', 'Fechamento consultivo', 'Urgência e condição especial', 'Onboarding após fechamento');
delete from public.growth_sales_objections where objection in ('Está caro.', 'Não tenho tempo.', 'Já tentei antes e não funcionou.', 'Prefiro esperar mais um pouco.', 'Tenho insegurança / medo de não conseguir.');
delete from public.growth_winning_phrases where phrase in ('Você não precisa se adaptar à turma. O curso se adapta a você.', 'Aqui, você fala desde o primeiro dia.', 'Relatórios de IA mostram sua evolução semanal de forma concreta.', 'Suporte 24h: você nunca está sozinho.', 'Acesso vitalício às conversações: fluência não se esquece.', 'Aqui você paga por resultado, não por anos preso em uma turma sem evolução.', 'Flexibilidade e reposição ilimitada garantem que sua agenda não seja problema.', 'Nosso app e relatórios semanais mostram sua evolução com clareza.');

insert into public.growth_sales_plans (name, description, price, ideal_for, benefits, recommended_when, active, updated_at) values
...
insert into public.growth_sales_personas (name, age_range, profile, goals, pains, recommended_plan, active, updated_at) values
...
insert into public.growth_sales_scripts (title, stage, name, type, content, examples, when_to_use, what_to_avoid, avoid, order_index, active, updated_at) values
...
insert into public.growth_sales_objections (objection, category, recommended_response, deepening_question, closing_phrase, active, updated_at) values
...
insert into public.growth_winning_phrases (phrase, context, stage, source, usage_count, positive_feedback_count, positive_count, active, updated_at) values
