...
(() => {
  ...
  function create({ root, fetchWithAuth, currentCompetencia }) {
    ...
    const render = () => {
      ...
      root.innerHTML = `${header()}${data.sourceUpdatedAt ? `<p role="status">Dados da última sincronização completa: ${escape(new Date(data.sourceUpdatedAt).toLocaleString('pt-BR', {timeZone:'America/Sao_Paulo'}))}.${data.sourceStale ? ' Aguardando atualização do CRM.' : ''}</p>` : ''}<div class="space-goals-kpis">${cards.map(([label, value]) => `<article class="space-goals-card"><span>${label}</span><strong>${value}</strong></article>`).join('')}</div>
