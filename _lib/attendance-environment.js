...
const assertAttendanceOperation = (env, options, targets = policy) => {
  ...
  if (options.environment !== 'staging' || options.confirm !== true
    || env.ATTENDANCE_STAGING_CONFIRM !== `staging:${target.ref}`) deny('confirmation_required');
  let db;
  try { db = new URL(required(env, 'ATTENDANCE_STAGING_DATABASE_URL')); } catch { deny('invalid_database_target'); }
