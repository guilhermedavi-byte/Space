...
const setRole = (role) => {
  ...
  if (currentRole === "teacher") {
    // Keep the sidebar pending badge up to date even if the user doesn't open the section.
    renderTeacherPedagogico().catch(() => {});
  }
...
const closeModal = () => {
  ...
  // Ensure pedagogical autosave timers are cleaned up when closing via X / backdrop.
  if (lessonLogAutosaveTimer) window.clearInterval(lessonLogAutosaveTimer);
  lessonLogAutosaveTimer = null;
  activeLessonLogDraft = null;
  activeLessonLogMeta = null;
  lessonLogDirty = false;
...
let activeLessonLogDraft = null;
let activeLessonLogMeta = null; // { lesson, log }
let lessonLogAutosaveTimer = null;
let lessonLogDirty = false;

const PED_TOPICS = ["Gramática", "Vocabulário", "Pronúncia", "Conversação", "Escrita", "Compreensão"];

const normalizeLessonLogDraft = (raw) => {
  const src = raw && typeof raw === "object" ? raw : {};
  return {
    eventId: String(src.eventId || "").trim(),
    professorId: String(src.professorId || "").trim(),
    alunoId: String(src.alunoId || "").trim(),
    dateKey: String(src.dateKey || "").trim(),
    statusAula: String(src.statusAula || "").trim(),
    novaDataAcordada: String(src.novaDataAcordada || "").trim(),
    oQueFoiTrabalhado: String(src.oQueFoiTrabalhado || "").trim(),
    nivelDificuldade: String(src.nivelDificuldade || "adequado").trim(),
    topicosAbordados: Array.isArray(src.topicosAbordados) ? src.topicosAbordados.map((t) => String(t || "").trim()).filter(Boolean) : [],
    engajamento: Number(src.engajamento) || 0,
    notaAula: src.notaAula == null ? "" : String(src.notaAula),
    pontosFortesAluno: String(src.pontosFortesAluno || "").trim(),
    pontosADesenvolver: String(src.pontosADesenvolver || "").trim(),
    feedbackParaAluno: String(src.feedbackParaAluno || "").trim(),
    temaProximaAula: String(src.temaProximaAula || "").trim(),
    tarefaDeCasa: String(src.tarefaDeCasa || "").trim(),
    observacoesInternas: String(src.observacoesInternas || "").trim(),
    nivelCEFR: String(src.nivelCEFR || "").trim(),
    evolucao: String(src.evolucao || "").trim(),
  };
};

const setLessonLogAutosaveLabel = (text) => {
  const el = modalBody?.querySelector("[data-ped-autosave]");
  if (el instanceof HTMLElement) el.textContent = String(text || "");
};

const setLessonLogDirty = (dirty) => {
  lessonLogDirty = Boolean(dirty);
  if (!lessonLogDirty) {
    setLessonLogAutosaveLabel("Salvo");
    return;
  }
  setLessonLogAutosaveLabel("Alterações não salvas");
};

const syncLessonLogSectionsVisibility = () => {
  if (!modalBody || !activeLessonLogDraft) return;
  const status = String(activeLessonLogDraft.statusAula || "").trim().toLowerCase();
  const contentBlocks = modalBody.querySelectorAll("[data-ped-hide-when]");
  contentBlocks.forEach((block) => {
    if (!(block instanceof HTMLElement)) return;
    const hideWhen = String(block.getAttribute("data-ped-hide-when") || "").trim().toLowerCase();
    if (!hideWhen) return;
    const shouldHide = hideWhen.split(",").map((s) => s.trim()).includes(status);
    block.hidden = shouldHide;
  });

  const remarcada = status === "remarcada";
  const resched = modalBody.querySelector("[data-ped-reschedule]");
  if (resched instanceof HTMLElement) resched.hidden = !remarcada;
};

const renderLessonLogModalBody = ({ lesson, draft } = {}) => {
  const safeLesson = lesson && typeof lesson === "object" ? lesson : {};
  const d = normalizeLessonLogDraft(draft);

  const dateLabel = formatBrShortDate(safeLesson.dateKey);
  const timeLabel = formatTimeRangeFromMinutes(safeLesson.startMin, safeLesson.endMin);
  const student = String(safeLesson.title || "Aluno").trim();

  const topicChips = PED_TOPICS.map((topic) => {
    const active = d.topicosAbordados.includes(topic);
    return `<button type="button" class="ped-chip ${active ? "is-active" : ""}" data-ped-topic="${escapeHtml(topic)}">${escapeHtml(topic)}</button>`;
  }).join("");

  const stars = Array.from({ length: 5 }).map((_, idx) => {
    const value = idx + 1;
    const active = Number(d.engajamento) >= value;
    return `<button type="button" class="ped-star ${active ? "is-active" : ""}" data-ped-star="${value}" aria-label="${value} estrelas">★</button>`;
  }).join("");

  return `
    <div class="modal-form">
      <div class="ped-log-statusbar">
        <div class="ped-log-status">📅 ${escapeHtml(dateLabel)} · ${escapeHtml(timeLabel)} · ${escapeHtml(student)}</div>
        <div class="ped-autosave" data-ped-autosave>—</div>
      </div>

      <div class="modal-inline-error" data-ped-error hidden></div>

      <div class="ped-section">
        <div class="ped-section-title">Presença</div>
        <div class="ped-radio-grid">
          ${[
            ["realizada", "Realizada"],
            ...
            .map(([value, label]) => {
              const checked = String(d.statusAula || "") === value ? "checked" : "";
              return `
                <label class="ped-radio">
                  <input type="radio" name="ped-status" value="${escapeHtml(value)}" data-ped-status ${checked} />
                  <span>${escapeHtml(label)}</span>
                </label>
              `;
            })
        ...
        <div class="modal-row" data-ped-reschedule hidden style="grid-template-columns: minmax(0, 1fr); margin-top: 12px;">
          <label class="modal-field">
            <span>Nova data acordada</span>
            <input class="modal-input" type="date" data-ped-field="novaDataAcordada" value="${escapeHtml(d.novaDataAcordada)}" />
          </label>
        </div>
      </div>

      <div class="ped-section" data-ped-hide-when="falta,cancelada,remarcada">
        <div class="ped-section-title">Conteúdo da aula</div>
        <label class="modal-field">
          <span>O que foi trabalhado hoje</span>
          <textarea class="modal-textarea" data-ped-field="oQueFoiTrabalhado" placeholder="Descreva o que foi feito na aula...">${escapeHtml(
            d.oQueFoiTrabalhado
          )}</textarea>
        </label>
        ...
        <label class="modal-field">
          ...
          <select class="modal-input" data-ped-field="nivelDificuldade">
            <option value="muito_facil" ${d.nivelDificuldade === "muito_facil" ? "selected" : ""}>Muito fácil</option>
            <option value="adequado" ${d.nivelDificuldade === "adequado" ? "selected" : ""}>Adequado</option>
            <option value="desafiador" ${d.nivelDificuldade === "desafiador" ? "selected" : ""}>Desafiador</option>
            <option value="muito_dificil" ${d.nivelDificuldade === "muito_dificil" ? "selected" : ""}>Muito difícil</option>
          </select>
        </label>

        <div class="modal-field">
          <span>Tópicos abordados</span>
          <div class="ped-chip-row" data-ped-topics>${topicChips}</div>
        </div>
      </div>

      <div class="ped-section" data-ped-hide-when="falta,cancelada,remarcada">
        <div class="ped-section-title">Avaliação do aluno</div>
        <div class="modal-row" style="grid-template-columns: minmax(0, 1fr) 160px; gap: 12px;">
          <div class="modal-field">
            <span>Engajamento do aluno</span>
            <div class="ped-stars" data-ped-stars>${stars}</div>
          </div>
          <label class="modal-field">
            <span>Nota da aula (0–10)</span>
            <input class="modal-input" type="number" min="0" max="10" step="0.1" data-ped-field="notaAula" value="${escapeHtml(d.notaAula)}" />
          </label>
        </div>

        <label class="modal-field">
          <span>Pontos fortes observados</span>
          <textarea class="modal-textarea" data-ped-field="pontosFortesAluno" placeholder="Ex: boa pronúncia...">${escapeHtml(
            d.pontosFortesAluno
          )}</textarea>
        </label>
        <label class="modal-field">
          <span>Pontos a desenvolver</span>
          <textarea class="modal-textarea" data-ped-field="pontosADesenvolver" placeholder="Ex: artigos definidos...">${escapeHtml(
            d.pontosADesenvolver
          )}</textarea>
        </label>
        <label class="modal-field">
          <span>Feedback para o aluno</span>
          <textarea class="modal-textarea" data-ped-field="feedbackParaAluno" placeholder="Ex: parabéns pelo progresso...">${escapeHtml(
            d.feedbackParaAluno
          )}</textarea>
        </label>
      </div>
      ...
      <div class="ped-section" data-ped-hide-when="falta,cancelada,remarcada">
        <div class="ped-section-title">Próximos passos</div>
        <label class="modal-field">
          <span>Tema da próxima aula</span>
          <input class="modal-input" type="text" data-ped-field="temaProximaAula" value="${escapeHtml(d.temaProximaAula)}" />
        </label>
        <label class="modal-field">
          <span>Tarefa para casa</span>
          <textarea class="modal-textarea" data-ped-field="tarefaDeCasa" placeholder="Ex: ler o capítulo 3...">${escapeHtml(
            d.tarefaDeCasa
          )}</textarea>
        </label>
        <label class="modal-field">
          <span>Observações internas</span>
          <textarea class="modal-textarea" data-ped-field="observacoesInternas" placeholder="Visível apenas para professor e admin.">${escapeHtml(
      ...
      <div class="ped-section">
        <div class="ped-section-title">Progresso do aluno (opcional)</div>
        <label class="modal-field">
          <span>Nível atual do aluno (CEFR)</span>
          <select class="modal-input" data-ped-field="nivelCEFR">
            <option value="" ${d.nivelCEFR ? "" : "selected"}>—</option>
            <option value="A1" ${d.nivelCEFR === "A1" ? "selected" : ""}>A1</option>
            <option value="A2" ${d.nivelCEFR === "A2" ? "selected" : ""}>A2</option>
            <option value="B1" ${d.nivelCEFR === "B1" ? "selected" : ""}>B1</option>
            <option value="B2" ${d.nivelCEFR === "B2" ? "selected" : ""}>B2</option>
            <option value="C1" ${d.nivelCEFR === "C1" ? "selected" : ""}>C1</option>
            <option value="C2" ${d.nivelCEFR === "C2" ? "selected" : ""}>C2</option>
          </select>
        </label>
        <label class="modal-field">
          <span>Evolução desde a última avaliação</span>
          <select class="modal-input" data-ped-field="evolucao">
            <option value="" ${d.evolucao ? "" : "selected"}>—</option>
            <option value="regressou" ${d.evolucao === "regressou" ? "selected" : ""}>Regressou</option>
            <option value="estavel" ${d.evolucao === "estavel" ? "selected" : ""}>Estável</option>
            <option value="evoluiu" ${d.evolucao === "evoluiu" ? "selected" : ""}>Evoluiu</option>
            <option value="evoluiu_muito" ${d.evolucao === "evoluiu_muito" ? "selected" : ""}>Evoluiu muito</option>
          </select>
        </label>
      </div>
    </div>
...
const openTeacherLessonLogModal = ({ lesson, log } = {}) => {
  if (!lesson) return;

  activeLessonLogMeta = { lesson, log: log || null };
  activeLessonLogDraft = normalizeLessonLogDraft({
    ...(log?.payload && typeof log.payload === "object" ? log.payload : {}),
    eventId: String(lesson.id || "").trim(),
    professorId: sessionUser?.id || "",
    alunoId: String(lesson.alunoId || "").trim(),
    dateKey: String(lesson.dateKey || "").trim(),
  });

  setLessonLogDirty(false);
  ...
  const startAutosave = () => {
    stopAutosave();
    lessonLogAutosaveTimer = window.setInterval(() => {
      if (!lessonLogDirty) return;
      if (!activeLessonLogDraft || !activeLessonLogDraft.statusAula) return;
      saveActiveLessonLog({ silent: true });
    }, 30_000);
  };

  openModal({
    title: "Registro pedagógico",
    bodyHtml: renderLessonLogModalBody({ lesson, draft: activeLessonLogDraft }),
    ...
    onPrimary: () => {
      saveActiveLessonLog({ silent: false });
      return false;
    },
  });

  syncLessonLogSectionsVisibility();
  setLessonLogAutosaveLabel("—");
  startAutosave();
};

const saveActiveLessonLog = async ({ silent } = {}) => {
  if (!activeLessonLogDraft || !activeLessonLogMeta?.lesson) return;
  const errorEl = modalBody?.querySelector("[data-ped-error]");
  if (errorEl instanceof HTMLElement) {
    errorEl.hidden = true;
    errorEl.textContent = "";
  }

  const statusAula = String(activeLessonLogDraft.statusAula || "").trim();
  if (!statusAula) {
    if (!silent && errorEl instanceof HTMLElement) {
      errorEl.hidden = false;
      errorEl.textContent = "Selecione o status da aula para salvar.";
    }
    return;
  }

  setLessonLogAutosaveLabel("Salvando…");
  try {
    const payload = {
      ...activeLessonLogDraft,
      statusAula,
      notaAula: activeLessonLogDraft.notaAula === "" ? null : Number(activeLessonLogDraft.notaAula),
      engajamento: Number(activeLessonLogDraft.engajamento) || 0,
    };
    ...
    if (!res.ok) {
      // eslint-disable-next-line no-console
      console.error("[pedagogico] save failed", { status: res.status, data });
      if (!silent && errorEl instanceof HTMLElement) {
        errorEl.hidden = false;
        errorEl.textContent = "Não foi possível salvar agora. Tente novamente.";
      }
      setLessonLogAutosaveLabel("Erro ao salvar");
      return;
    }
    ...
    // Refresh list state so the badge/list updates immediately.
    await renderTeacherPedagogico({ force: true });
  } catch (error) {
    // eslint-disable-next-line no-console
...
// Controle Pedagógico (Professor)
const teacherPedagogicoList = document.querySelector("[data-teacher-log-list]");
const teacherPedagogicoEmpty = document.querySelector("[data-teacher-log-empty]");
const teacherPedagogicoError = document.querySelector("[data-teacher-log-error]");
const teacherPedagogicoStatus = document.querySelector("[data-teacher-log-status]");
const teacherPedagogicoSidebarBadge = document.querySelector("[data-teacher-pedagogico-badge]");

let teacherPedagogicoState = {
  isLoading: false,
  loadedAt: 0,
  lessons: [],
...
const formatBrShortDate = (dateKey) => {
  const raw = String(dateKey || "");
...
const formatTimeRangeFromMinutes = (startMin, endMin) => {
  const start = formatHmFromMinutes(startMin);
  const end = formatHmFromMinutes(endMin);
  return `${start}–${end}`;
};

const getLessonStartDate = (lesson) => {
  if (!lesson) return null;
  const date = parseDateKey(lesson.dateKey);
  if (!date) return null;
  return buildDateFromDateKeyAndMinutes(lesson.dateKey, Number(lesson.startMin) || 0);
};

const getSaoPauloNowSafe = () => {
  try {
    if (typeof getSaoPauloNow === "function") return getSaoPauloNow();
  } catch (e) {
    // ignore
  }
  return new Date();
...
const isPastLesson = (lesson) => {
  ...
  return start.getTime() < getSaoPauloNowSafe().getTime();
};

const getPedagogicoBadgeMeta = ({ lesson, log }) => {
  const past = isPastLesson(lesson);
  if (!past) return { label: "Aula futura", className: "is-future" };
  const statusAula = String(log?.payload?.statusAula || log?.statusAula || "").trim();
  if (statusAula) return { label: "Concluída", className: "is-done" };
  return { label: "Em aberto", className: "is-open" };
};

const setTeacherPedagogicoSidebarBadge = (count) => {
  if (!(teacherPedagogicoSidebarBadge instanceof HTMLElement)) return;
  const safe = Math.max(0, Number(count) || 0);
  teacherPedagogicoSidebarBadge.textContent = String(safe);
  teacherPedagogicoSidebarBadge.hidden = safe <= 0;
};

const renderTeacherPedagogicoList = () => {
  if (!(teacherPedagogicoList instanceof HTMLElement)) return;

  const lessons = Array.isArray(teacherPedagogicoState.lessons) ? teacherPedagogicoState.lessons : [];
  const logs = teacherPedagogicoState.logsByEventId instanceof Map ? teacherPedagogicoState.logsByEventId : new Map();

  const items = lessons
    .slice()
    .sort((a, b) => (a.dateKey === b.dateKey ? (a.startMin || 0) - (b.startMin || 0) : String(a.dateKey).localeCompare(String(b.dateKey))))
    .map((lesson) => {
      const log = logs.get(String(lesson.id || "").trim()) || null;
      const meta = getPedagogicoBadgeMeta({ lesson, log });
      const isFuture = meta.className === "is-future";
      const dateLabel = formatBrShortDate(lesson.dateKey);
      const timeLabel = formatTimeRangeFromMinutes(lesson.startMin, lesson.endMin);
      const studentName = String(lesson.title || "Aluno").trim();
      const sub = "Registro pedagógico da aula";
      const pillText = meta.label.toUpperCase();

      return `
        <li>
          <button
            class="teacher-log-item ${isFuture ? "is-future" : ""}"
            type="button"
            data-pedagogico-open="${escapeHtml(String(lesson.id || ""))}"
            ${isFuture ? "disabled" : ""}
            aria-label="Abrir registro pedagógico"
          >
            <div class="teacher-log-item-left">
              <div class="teacher-log-item-top">
                <span>📅 ${escapeHtml(dateLabel)} · ${escapeHtml(timeLabel)}</span>
                <span>${escapeHtml(studentName)}</span>
              </div>
              <div class="teacher-log-item-sub">${escapeHtml(sub)}</div>
            </div>
            <span class="teacher-log-pill ${escapeHtml(meta.className)}">${escapeHtml(pillText)}</span>
          </button>
        </li>
      `;
    })
  ...
  teacherPedagogicoList.innerHTML = items;

  if (teacherPedagogicoEmpty instanceof HTMLElement) {
    teacherPedagogicoEmpty.hidden = lessons.length > 0;
  }
};

const loadTeacherPedagogicoData = async () => {
  const resLessons = await fetchWithAuth("/api/schedule-events", { method: "GET" });
  if (!resLessons.ok) throw new Error("lessons_fetch_failed");
  const dataLessons = await resLessons.json().catch(() => null);
  const events = Array.isArray(dataLessons?.events) ? dataLessons.events : [];
  const lessons = events.filter((evt) => evt && typeof evt === "object" && evt.type === "lesson");

  const resLogs = await fetchWithAuth("/api/lesson-logs", { method: "GET" });
  const dataLogs = await resLogs.json().catch(() => null);
  const logsArr = Array.isArray(dataLogs?.logs) ? dataLogs.logs : [];
  const logsByEventId = new Map();
  logsArr.forEach((log) => {
    const eid = String(log?.eventId || "").trim();
    if (eid) logsByEventId.set(eid, log);
  });

  return { lessons, logsByEventId };
};

const computePedagogicoPendingCount = ({ lessons, logsByEventId }) => {
  const arr = Array.isArray(lessons) ? lessons : [];
  const map = logsByEventId instanceof Map ? logsByEventId : new Map();
  let open = 0;
  arr.forEach((lesson) => {
    if (!isPastLesson(lesson)) return;
    const log = map.get(String(lesson.id || "").trim()) || null;
    const statusAula = String(log?.payload?.statusAula || log?.statusAula || "").trim();
    if (!statusAula) open += 1;
  });
  return open;
};

const renderTeacherPedagogico = async ({ force = false } = {}) => {
  if (currentRole !== "teacher") return;
  if (!(teacherPedagogicoList instanceof HTMLElement)) return;
  if (teacherPedagogicoState.isLoading) return;

  const now = Date.now();
  if (!force && teacherPedagogicoState.loadedAt && now - teacherPedagogicoState.loadedAt < 30_000) {
    renderTeacherPedagogicoList();
    return;
  }

  teacherPedagogicoState.isLoading = true;
  if (teacherPedagogicoStatus instanceof HTMLElement) teacherPedagogicoStatus.textContent = "Carregando…";
  if (teacherPedagogicoError instanceof HTMLElement) teacherPedagogicoError.hidden = true;

  try {
    const { lessons, logsByEventId } = await loadTeacherPedagogicoData();
    teacherPedagogicoState.lessons = lessons;
    teacherPedagogicoState.logsByEventId = logsByEventId;
    teacherPedagogicoState.loadedAt = Date.now();

    const pending = computePedagogicoPendingCount({ lessons, logsByEventId });
    setTeacherPedagogicoSidebarBadge(pending);

    renderTeacherPedagogicoList();
    if (teacherPedagogicoStatus instanceof HTMLElement) {
      teacherPedagogicoStatus.textContent = pending > 0 ? `${pending} em aberto` : "Tudo em dia";
    }
  } catch (error) {
    if (teacherPedagogicoError instanceof HTMLElement) teacherPedagogicoError.hidden = false;
    if (teacherPedagogicoStatus instanceof HTMLElement) teacherPedagogicoStatus.textContent = "";
  } finally {
    teacherPedagogicoState.isLoading = false;
  }
};
...
document.addEventListener("click", (event) => {
  ...
  if (target instanceof Element) {
    if (activeLessonLogDraft && modalBody && modalOverlay && !modalOverlay.hidden) {
      const topicBtn = target.closest("[data-ped-topic]");
      if (topicBtn instanceof HTMLButtonElement) {
        event.preventDefault();
        const topic = String(topicBtn.getAttribute("data-ped-topic") || "").trim();
        if (!topic) return;
        const current = Array.isArray(activeLessonLogDraft.topicosAbordados) ? activeLessonLogDraft.topicosAbordados : [];
        const next = current.includes(topic) ? current.filter((t) => t !== topic) : [...current, topic];
        activeLessonLogDraft.topicosAbordados = next;
        topicBtn.classList.toggle("is-active", next.includes(topic));
        setLessonLogDirty(true);
        return;
      }

      const starBtn = target.closest("[data-ped-star]");
      if (starBtn instanceof HTMLButtonElement) {
        event.preventDefault();
        const value = Number(starBtn.getAttribute("data-ped-star") || 0);
        const safe = Number.isFinite(value) ? Math.max(0, Math.min(5, value)) : 0;
        activeLessonLogDraft.engajamento = safe;
        modalBody.querySelectorAll("[data-ped-star]").forEach((btn) => {
          if (!(btn instanceof HTMLElement)) return;
          const v = Number(btn.getAttribute("data-ped-star") || 0);
          btn.classList.toggle("is-active", v <= safe);
        });
        setLessonLogDirty(true);
        return;
      }
    ...
    const openPed = target.closest("[data-pedagogico-open]");
    if (openPed instanceof HTMLButtonElement) {
      event.preventDefault();
      const id = String(openPed.getAttribute("data-pedagogico-open") || "").trim();
      if (!id) return;
      const lesson = (teacherPedagogicoState.lessons || []).find((l) => String(l.id || "").trim() === id);
      if (!lesson) return;
      const log = teacherPedagogicoState.logsByEventId instanceof Map ? teacherPedagogicoState.logsByEventId.get(id) : null;
      openTeacherLessonLogModal({ lesson, log });
      return;
    }
...
document.addEventListener("input", (event) => {
  if (!activeLessonLogDraft || !modalBody || modalOverlay?.hidden) return;
  const target = event.target;
  if (!(target instanceof Element)) return;

  const fieldEl = target.closest("[data-ped-field]");
  if (fieldEl instanceof HTMLInputElement || fieldEl instanceof HTMLTextAreaElement || fieldEl instanceof HTMLSelectElement) {
    const key = String(fieldEl.getAttribute("data-ped-field") || "").trim();
    if (!key) return;
    activeLessonLogDraft[key] = fieldEl.value;
    setLessonLogDirty(true);
    return;
  }
});
...
document.addEventListener("change", (event) => {
  ...
  if (target instanceof HTMLInputElement && target.matches("[data-ped-status]")) {
    activeLessonLogDraft.statusAula = target.value;
    syncLessonLogSectionsVisibility();
    setLessonLogDirty(true);
    return;
  }
  ...
  const fieldEl = target.closest("[data-ped-field]");
  if (fieldEl instanceof HTMLSelectElement || fieldEl instanceof HTMLInputElement) {
    const key = String(fieldEl.getAttribute("data-ped-field") || "").trim();
    if (!key) return;
    activeLessonLogDraft[key] = fieldEl.value;
    setLessonLogDirty(true);
