...
const makeInput=()=>{
 const businesses=['07','08','09'].map(m=>({id:'deal-'+m,total:100,wonAt:`2026-${m}-10T12:00:00-03:00`,stage:{name:'Fechado',pipeline:{name:'Conversão'}},sdrId:null}));
 return {businesses,users:[],growthPeople:[],goals:{'2026-07':{},'2026-08':{},'2026-09':{}},globalConfig:null,sdrEvents:[],sdrEventsComplete:true,identityKind:'dealId',source:{fetchCompleted:true,paginationCompleted:true,pagesFetched:1,expectedPages:1,recordsFetched:3,retryCount:0,datacrazy429Count:0},sdrAttribution:{status:'loaded',evidence:businesses.map(b=>({dealId:b.id,sdrId:null,source:'explicit_datacrazy_sdrId_null'}))}};
};
test('complete simulated execution uses real engine and sanitized output; no raw source PII',()=>{
 ...
 input.snapshots=Object.fromEntries(Object.entries(first.months).map(([key,m])=>[key,{snapshot:{monthlyIncludedDeals:m.ledger.rows,includedDeals:key==='2026-09'?first.weekly.rows:[],fetchCompleted:true,paginationCompleted:true},month:{period:{monthKey:key},summary:{realizado:100,totalVendas:1}},weekly:{team:{closers:{actualValue:key==='2026-09'?100:0,count:key==='2026-09'?1:0}}}}]));
...
test('incomplete Datacrazy and missing SDR evidence cannot produce PASS',()=>{
 ...
 const report=project(reconcileExport(input),input,createGuard({env}).counts);assert.equal(report.certification,'FAIL');assert.equal(report.sdrCertified,false);assert.equal(report.months['2026-09'].sdr.sdrNotLoadedDeals,1);assert.equal(report.months['2026-09'].sdr.sdrFinancialDelta,null);
});
...
test('complete worker simulation: real collector, Firestore read adapter, OAuth path, runtime and sanitization',()=>{
 ...
 const docs={};for(const [key,m] of Object.entries(first.months)){
  ...
  const p={snapshot:{monthlyIncludedDeals:m.ledger.rows,includedDeals:m.ledger.rows,fetchCompleted:true,paginationCompleted:true},month:{period:{monthKey:key},summary:{realizado:100,totalVendas:1}},weekly:{commercialWeek:require('./api/_lib/commercial-week').resolveCommercialWeek({now:key+'-10'}),team:{closers:{actualValue:100,count:1}}}};
 ...
 global.fetch=async(url,opts={})=>{
  ...
  if(url.hostname==='crm.example.test')return new Response(JSON.stringify({items:input.businesses,total:3}));
 ...
 const result=JSON.parse(output);assert.equal(result.certification,'PASS');assert.equal(result.authenticatedSourceLoaded,true);assert.equal(result.writesAttempted,0);assert.equal(result.source.pages,1);assert.equal(result.months['2026-09'].sdr.sdrNoAttributionDeals,1);
