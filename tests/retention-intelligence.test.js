...
test('1 Score parcial diferencia zero real e informação inexistente',()=>{assert.equal(H.scoreHealth({}).health_score,null);const a=H.scoreHealth({financial_status:'overdue',days_overdue:45});assert.equal(a.health_score,10);assert.equal(a.score_coverage_pct,15);assert.equal(a.dimension_scores.teacher_pulse,null);const b=H.scoreHealth({support_observed:true,open_activities:8});assert.equal(b.health_score,null);assert.equal(b.score_coverage_pct,0);assert.equal(b.risk_factors.length,0);});
test('2 Limites de tiers e desvio contra baseline',()=>{assert.deepEqual([100,80,79,65,64,45,44,0,null].map(H.tier),['healthy','healthy','attention','attention','risk','risk','critical','critical','unknown']);assert.equal(H.scoreHealth({days_since_last_activity:1,engagement_change_pct:-80}).health_score,null);});
...
function ui(runtime={}){
 ...
 const healthy={...row,student_id:'healthy',nome:'Aluno saudável',health_score:100,health_tier:'healthy',score_coverage_pct:25,risk_factors:[],signals:{}};
...
test('9 Drawer contextual abre ficha e mantém Student 360',async()=>{const{dom,api,bridge}=ui(),d=dom.window.document;api.mount(d.querySelector('#root'),bridge);await ready();d.querySelector('[data-ri-case="a"]').click();assert.equal(d.querySelector('[data-ri-case-dialog]').open,true);assert.match(d.querySelector('[data-ri-case-dialog]').textContent,/Caso atual/);d.querySelector('[data-ri-case-dialog] [data-ri-student]').click();assert.equal(bridge.opened,'a');assert.equal(d.querySelector('[data-ri-case-dialog]').open,false);await api.hydrateStudent(d.querySelector('#sheet'),'a',bridge.fetch);assert.match(d.querySelector('#sheet').textContent,/valor observado: 15/);});
test('10 Comandos preservam nós e eventos; Analytics é view secundária',async()=>{const{dom,api,bridge}=ui(),d=dom.window.document,old=d.querySelector('#old-action');let clicks=0;old.onclick=()=>clicks++;api.mount(d.querySelector('#root'),bridge);await ready();d.querySelector('[data-ri-go=commands]').click();assert.equal(d.querySelector('#old-action'),old);old.click();assert.equal(clicks,1);assert.equal(d.querySelector('[data-ri-operation]').hidden,false);d.querySelector('[data-ri-go=analytics]').click();assert.equal(d.querySelector('[data-ri-operation]').hidden,true);assert.match(d.querySelector('[data-ri-panel]').textContent,/Performance/);});
