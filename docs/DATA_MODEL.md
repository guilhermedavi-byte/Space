...
| `pedagogico_pending_writes` | 433 | Firestore | `legacy-active` | pseudo-outbox de payload antes do insert Supabase | `api/_lib/live-lessons.js:441` | leitura operacional indireta / auditoria | não é backlog confiável hoje: cria duplicatas por tentativa e o estado `supabase_saved` não é garantido |
